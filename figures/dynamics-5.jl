...
# fix types
# version  5: random numbers in full diagonal

# arguments: size S, sigma for random diagonal, #runs, index
...
function matrixdiagonal(size, sd) 
 a = Matrix{Float64}(undef, size, size)
 y = fill!(a, 2) + (-1)*I
 d = sd*randn((size))
 for i in 1:size
...
# runs some number of times, assuming only a single species at minimum
#computes statistics
#arguments: matrix size, standard deviation of off diagonal, number of runs

function dynamicsstats(size, v, count, index) #size, count as strings
    m = Symmetric(matrixdiagonal(size, v))
    o = open(string("output"), "a")
