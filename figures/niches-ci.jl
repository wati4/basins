...
# initial conditions are gaussian in positive orthant with scale fixed.

# parameters are n = lattice size, l = system size, #runs, scale, index
# 9/10/23: put in gaps, canonical form
# 9/20/24: incorporates scale.
...
function dynamics(n, l, count, scale, index) #n, count as integers
    m = fixmatrix(n, l)
    o = open(string("output"), "a")
    write(o, "matrix[" * string(index) * "] =" * string(m, "\n"))
...
#last argument is scale

dynamics(parse(Int, ARGS[1]),parse(Float64, ARGS[2]),parse(Int, ARGS[3]),parse(Float64, ARGS[4]),parse(Int, ARGS[5]))
